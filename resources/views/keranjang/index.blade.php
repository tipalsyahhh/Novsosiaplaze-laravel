...
<div class="judul-keranjang">
    <center>
       ...
       <div class="btn-market">
        <a href="{{ route('pages.welcome') }}" style="text-decoration: none;">Kembali</a>
       </div>
    </center>
</div>
...
<div class="keranjang">
    ...
    <div class="card-keranjang">
        <div class="nama-toko" style="display: flex;">
        ...
        <div class="isi-keranjang">
            ...
            @endif
            <p>{{ Str::limit($item->postingan->deskripsi, 22, '...') }}</p>
            ...
            <form action="{{ route('keranjang.hapus', ['id' => $item->id]) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit" class="button-keranjang" style="margin-top: 15px;"><i
                        class="bi bi-trash-fill"></i></button>
            </form>
        </div>
        <h5 style="margin-top: -5px; color: #333;">{{ $item->postingan->nama_menu }}</h5>
        <h5 style="color: #000; font-weight: bold;">Rp {{ number_format($item->postingan->harga, 0, ',', '.') }}
            </h5>
    </div>
    @endforeach
    @else
    <div class="no-data">
        <img src="{{ asset('img/no-keranjang.png') }}" alt="ini-gif" class="gif-data">
        <center>
            <h6 style="margin-top: 15px;">Anda belum memiliki isi keranjang</h6>
        </center>
    </div>
    @endif
</div>
