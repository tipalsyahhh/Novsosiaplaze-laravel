...
<div class="container">
    <div class="row justify-content-center" style="width: 100%;">
        <div class="col-md-8">
            <div class="d-flex align-items-center" style="margin-bottom:20px; margin-top: 25px;">
            ...
            <div id="status-{{ $status->id }}" class="modal-status">
                <div class="modal-body">
                    <a href="{{ auth()->user()->id == $status->user_id ? route('dataAkun.index') : route('follow.profile', ['userId' => $status->user_id]) }}">
        ...
        <div id="likedUsersModal-{{ $status->id }}" class="modal">
            <div class="modal-asu-like">
                ...
                <div class="judul-like">
                    <h6 style="color: #333; margin-right: auto">di sukai oleh</h6>
                    <h6 style="color: 333;">{{ optional($status->like())->count() }} suka</h6>
                </div>
                                ...
                                <div class="modal-like">
                                    <div class="name-user">
                                        <a href="{{ auth()->user()->id == $like->user_id ? route('dataAkun.index') : route('follow.profile', ['userId' => $like->user_id]) }}" style="text-decoration: none;">
                                            <img src="{{ $like->user->profileImage ? asset('storage/' . $like->user->profileImage->image_path) : asset('storage/default_profile.png') }}" alt="Profile Image" class="rounded-circle" style="width: 40px; height: 40px;">
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        likeSpans.forEach(function (likeSpan) {
            likeSpan.addEventListener('click', function () {
                ...
                // Periksa apakah userId dan statusId tidak null atau undefined
                if (userId !== null && userId !== undefined && statusId !== null && statusId !== undefined) {
                    ...
                    // Gantilah dengan logika atau perintah untuk menampilkan daftar user yang melakukan like
                    console.log('Teks Likes diklik');

                    // Munculkan modal jika ditemukan
                    if (modal) {
...
<script>
    // Fungsi untuk memutar suara saat tombol "Like" diklik
